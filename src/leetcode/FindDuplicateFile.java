...
public class FindDuplicateFile {
	...
	public List<List<String>> findDuplicate(String[] paths) {
		...
		for (String path : paths) {
			...
			for (int i = 1; i <= pathAndFiles.length - 1; ++i) {
				String[] fileAndContent = pathAndFiles[i].split("[()]");
				String absPath = dir + "/" + fileAndContent[0];
				String content = fileAndContent[1];
				
				ArrayList<String> as;
				if (hmp.containsKey(content)) {
					hmp.get(content).add(absPath);
				} else {
		...
		for (String key : hmp.keySet()) {
			ArrayList<String> tmp = hmp.get(key);
			if (tmp.size() > 1)
				lls.add(hmp.get(key));
		}
		return lls;
